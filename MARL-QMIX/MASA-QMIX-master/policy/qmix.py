...
class QMIX:
    def __init__(self, args, writer=None):
        ...
        if self.args.load_model:
            if os.path.exists(self.model_dir + '/65_rnn_net_params.pkl'):
                path_rnn = self.model_dir + '/65_rnn_net_params.pkl'
                path_qmix = self.model_dir + '/65_qmix_net_params.pkl'
        ...
        if args.optimizer == "Adam":
            self.optimizer = torch.optim.Adam(self.eval_parameters, lr=args.lr, betas=(0.9, 0.999))
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        if self.writer is not None:
            ...
            for agent_id in range(self.n_agents):
                # 聚合时间步的 Q 值，记录每个 train_step 的 Q 值
                q_value_mean = q_evals[:, :, agent_id].mean().item()  # 对所有时间步和批次求均值
                self.writer.add_scalar(
                    f"Train/Agent_{agent_id}_Q_Values",
                    q_value_mean,  # 聚合后的 Q 值
                    self.train_step,
                )

            # 记录全局 Q 值
            global_q_value_mean = q_total_eval.mean().item()  # 对整个 episode 求均值
            self.writer.add_scalar(
                f"Train/Global_Q_Value",
                global_q_value_mean,
                self.train_step,
            )
